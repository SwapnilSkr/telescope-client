...
export function PostDetail({ post }: PostDetailProps) {
  ...
  const fetchPreviousMessage = async () => {
    if (!currentPost.has_previous) return;
    
    setLoadingPrevious(true);
    try {
      const response = await fetch(
        `${BASE_URL}/messages/previous?group_id=${currentPost.group_id}&message_id=${currentPost.message_id}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="space-y-6 w-full max-w-md p-6">
            <div className="flex justify-between items-center">
              ...
              <Skeleton className="h-6 w-32 bg-gray-700/30" />
            </div>
            ...
            <Skeleton className="h-40 w-full rounded-lg bg-gray-700/20" />
          </div>
        </div>
      ) : (
        <>
          <ScrollArea
            className="flex-grow w-full overflow-y-auto text-white px-4"
            style={{
              background:
            ...
            <div className="p-6 space-y-8">
              <div className="flex flex-col space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-3xl font-bold text-white">{currentPost.channel}</h2>
                  
                  <div className="flex items-center gap-2">
                    {currentPost.is_verified ? (
                      <Badge className="bg-green-600/20 text-green-500 hover:bg-green-600/30 flex items-center gap-1">
                        <svg
                      ...
                      <Badge variant="outline" className="text-yellow-500 border-yellow-500/50 flex items-center gap-1">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="16"
                          height="16"
                ...
                <div className="space-y-3 bg-[#1A1E38]/50 rounded-lg p-4 border border-gray-700/50">
                  {currentPost.description && (
                    <div className="space-y-1">
                      <h3 className="text-sm font-medium text-gray-400">ABOUT</h3>
                      <p className="text-sm text-gray-300">{currentPost.description}</p>
                    </div>
                  )}
                ...
                <div className="flex justify-between items-center">
                  <div className="space-y-2">
                    ...
                    {renderThreatStrength(getThreatLevel(currentPost.id))}
                  </div>
                  ...
                  <div className="w-full overflow-auto">
                    {currentPost.media.type === "image" && (
                      <div className="w-full max-h-[500px] overflow-hidden rounded-lg">
                        <Image
                          src={currentPost.media.url || "/placeholder.svg"}
                          alt={currentPost.media.alt || "Post image"}
                          width={800}
                          height={600}
                    ...
                    {currentPost.media.type === "video" && (
                      <div className="w-full max-h-[500px] rounded-lg">
                        <video
                          controls
                          poster={currentPost.media.thumbnail}
                          className="w-full h-auto max-h-[500px]"
                        >
                          <source src={currentPost.media.url} type="video/mp4" />
                          Your browser does not support the video tag.
                        </video>
                      </div>
                    )}
                    {currentPost.media.type === "file" && (
                      <Button
                        asChild
                        className="bg-purple-900 hover:bg-purple-800 text-white"
                      >
                        <a
                          href={currentPost.media.url}
                          download={currentPost.media.name}
          ...
          {currentPost.has_previous && (
            <div className="p-4 border-t border-gray-700/50">
              <Button
                onClick={fetchPreviousMessage}
                disabled={loadingPrevious}
                className="w-full bg-[#2D2D44] hover:bg-[#3D3D54] text-white"
              >
